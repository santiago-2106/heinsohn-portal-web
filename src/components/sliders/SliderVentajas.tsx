          ...
          {cards.map((card, index) => {
            ...
            return (
              <div
                ...
                className={`relative flex-shrink-0 w-[280px] md:w-[320px] h-[400px] snap-center border p-8 md:p-10 flex flex-col transition-all duration-500 cursor-pointer overflow-hidden ${isActive ? "border-text-title bg-bg-card-2" : "border-border-ui bg-transparent"
                  }`}
              >
                ...
                */}
                <div className={`flex flex-col w-full h-full transition-all duration-500 ${isActive ? "justify-start" : "justify-end"
                  }`}>

                  {/* Ícono (Cambia de color y tamaño dependiendo si está activa) */}
                  <div className={`material-symbols-rounded transition-all duration-500 ${isActive
                  ...
                  <h3 className={`font-light leading-snug transition-colors duration-300 ${isActive
                  ...
                  <div className={`transition-all duration-500 overflow-hidden ${isActive ? "max-h-[200px] opacity-100" : "max-h-0 opacity-0"
                    }`}>
          ...
          <div className="flex items-center gap-2">
            {cards.map((_, idx) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all duration-300 ${activeIndex === idx ? "w-4 bg-text-title" : "w-1.5 bg-border-ui hover:bg-text-body"
                  }`}
              />
            ))}
          </div>
