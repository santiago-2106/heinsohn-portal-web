...
// 1. Importa el archivo JSON (Ajusta la ruta según las opciones de arriba)
import BLOG_CONTENT from '@/messages/es/blog-content.json';

// 2. Crea una interfaz para tipar tu JSON
interface BlogItem {
  id: string;
  title: string;
  content: string;
  ...
  return (
    <aside className="hidden lg:flex w-[288px] sticky top-10 flex-col gap-6 p-6 border border-border-ui h-fit">
      ...
      <div className="flex items-center gap-4 w-60">
        <img 
          src="/img/img-jessica.jpg" 
          className="w-18 h-18 object-cover brightness-75 contrast-75 grayscale-75" 
          alt="Jessica" 
        />
        <div className="flex flex-col">
          <span className="font-light text-text-body">Jessica Alejandra Alonso Perez</span>
      ...
      <nav className="flex flex-col gap-6 border-t border-border-ui">
        ...
        <ul className="flex flex-col gap-4 border-l border-border-ui">
          {/* 3. Usa la interfaz BlogItem en lugar de 'any' */}
          {BLOG_CONTENT.map((item: BlogItem) => (
            <li key={item.id}>
              <a 
                href={`#${item.id}`} 
